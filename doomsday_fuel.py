...
def lcmm(l):
    return functools.reduce(lambda x, y: lcm(x, y), l)
...
#def solution(m):
    ...
    # Find the matrix (sub matrix left) probability of non-terminal to terminal (absorbed)
    # and right sub matrix probability of non-terminal to non-terminal (not absorbed)
    # Find inverse of identity matrix - right sub matrix
    # Format answer with MCD